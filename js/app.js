...
const app = {
  ...
  render() {
    ...
    switch (this.screen) {
      ...
      // Dynamic Geometry (kind "dynamic", g8) — its own grouped entry off the
      // main map (DYNAMIC-GEO-PLAN.md), but bounced home like the station
      // routes below while CONFIG.dynamicLive is false (shipped dark,
      // unreviewed — her call, 2026-09-04). `?dynamic=1` opens it for review.
      case "dynamics":
        if (!dynamicVisible()) { renderHome(this, view); renderCustomizeLink(this, view); break; }
        renderDynamic(this, view); break;
      case "dynamic":
        if (!dynamicVisible()) { renderHome(this, view); renderCustomizeLink(this, view); break; }
